...
function Coin () {
    ...
    // Funcion para pasar de string a número usable matemáticamente con parseFloat() y para redondear con Math.round()
    ...
    const saveCoin = () => {
        ...
        if(savedCoins != null) {
            if(savedCoins.indexOf(coin.id) != -1) {
                localStorage.setItem('favCoins', JSON.stringify(savedCoins.filter(moneda => moneda !== coin.id)))
            } else {
